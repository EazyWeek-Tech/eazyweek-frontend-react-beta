...
const getCenterFromStorage = () => {
  ...
  const MasterLeadForm = () => {
    ...
    // Master rows come from the detail grid carrying their TRANSDETAILS RECID.
    const recID =
      Number(safe(row?.recid || row?.recID || params?.leadOppId || resolvedLeadOppId).trim() || 0) || 0;

    // Customer id (the customer already exists for a master rule) — used to prefill details.
...
useEffect(() => {
  ...
  return () => {
    ...
    /** ---------------- Prefill customer (Master rule: customer already exists) ----------------
     * The row only carries id/name/mobile, so fetch the full customer to fill email (and clean
     * first/last + centre). custID stays read-only; nothing here creates a customer. */
    useEffect(() => {
      const id = safe(resolvedCustID).trim();
      if (!id) return;
      let alive = true;
      (async () => {
        try {
          const resp = await postJson(`${API_BASE_URL}/api/Customer/FetchCustomerDetails`, { custID: id });
          const data = resp?.data ?? resp;   // wrapped { success, data:{...} }
          if (!alive || !data) return;
          setForm((p) => ({
            ...p,
            firstName:   safe(p.firstName)   || safe(data.firstName),
            lastName:    safe(p.lastName)    || safe(data.lastName),
            email:       safe(p.email)       || safe(data.email || data.emailID),
            mobile:      safe(p.mobile)      || safe(data.mobilePhone || data.mobileNo),
            countryCode: safe(p.countryCode) || safe(data.countryCode),
            centerCode:  safe(p.centerCode)  || safe(data.centerCode),
          }));
        } catch (e) {
          console.error("FetchCustomerDetails (master) failed", e);
        }
      })();
      return () => { alive = false; };
    }, [resolvedCustID]);
            ...
            <div className="formGrid3">
              <div className="col">
                <div className="field">
                  ...
                  <input className="inp" value={resolvedCustID || "—"} readOnly disabled />
                </div>
