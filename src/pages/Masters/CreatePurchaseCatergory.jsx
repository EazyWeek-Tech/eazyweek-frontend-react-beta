...
const CreatePurchaseCategory = () => {
  ...
  const addRow = (level) => {
    if (level === "Category") {
      ...
      if (!name) return alert("Please enter Purchase Category Name.");
      setCategories((prev) => {
  ...
  // ---- API call stub (replace endpoint when ready) ----
  const postCreation = async (payload) => {
    // TODO: Replace with your real API when shared.
    // Example: `${API_BASE_URL}/api/Purchase/CategoryCreation`
    const endpoint = `${API_BASE_URL}/api/Purchase/CategoryCreation`;
    try {
      ...
      // While API is not ready, also log payload for quick verification
      console.warn("Purchase Category payload (preview):", payload);
      throw err;
    }
  };
  ...
  // matches your routing
  const gotoList = () => navigate("/purchase-categories");
  const gotoMapping = () => navigate("/purchase-category-mapping");
  ...
  return (
    <>
      ...
      <div className="page">
        <div className="wrap">
          <div className="crumb">
            <a onClick={() => navigate("/purchase")} style={{ cursor: "pointer" }}>
              Purchase Category
            </a>{" "}
            &nbsp;>&nbsp; Creation of Purchase Category
          </div>

          {/* Header with primary actions (Submit & Save) */}
          <div className="headerbar">
            <h1 className="title">Creation of Purchase Category</h1>
                ...
                {active === "SubCategory" && (
                  <>
                    ...
                    <div className="btns">
                      ...
                      <button className="btn light" onClick={() => setSubCategoryInput({ name: "" })}>
                        Clear
                      </button>
                    </div>
